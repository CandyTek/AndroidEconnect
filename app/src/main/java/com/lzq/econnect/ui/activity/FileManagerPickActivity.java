...
public class FileManagerPickActivity extends BaseActivity implements FileFragment.FileClickListener {
	...
	private FrameLayout fileListContainer;
				...
				switch (which) {
					...
					case 4: //粘贴
						if (oldPath != null) {
							...
							{
								if (oldFile.isFile())
									FileUtils.copyFile(oldPath,mCurrentPath + "/" + oldFile.getName());
								else
									FileUtils.copyFolder(oldPath,mCurrentPath + "/" + oldFile.getName());
								if (!isCopy)//如果是剪切
								{
									FileUtils.deleteFile(oldFile);//删除原文件
									isCopy = true;
								}
						...
						break;
					case 5://删除
						FileUtils.deleteFile(longClickFile);
						refreshCurrentPath(mCurrentPath);
						break;
					case 6://选择发送
